...
import '../styles/TablaCentros.css';
 ...
 const EntradaPrivadoForm = () => {
  ...
  const { createEntradaPrivado } = useMovimiento();
  ...
  const handleEntrarSubmit = async (e) => {
    ...
    try {
      const result = await createEntradaPrivado(newMovimiento);
  ...
  return (
    <div>
     ...
     <div>
        ...
        <div className='d-flex'>
        <button type="submit" className="btn btn-primary btn-lg mt-5 mb-3">Add Medicine</button>
       </div>
       <div className='card-footer'>
       <div className='text-center justify-content-center mx-4 mb-3 mb-lg-4'>
        <button className="btn  btn-danger btn-lg mt-3" onClick={handleEntrarSubmit}>Guardar Compra</button>
        </div>
        </div>
      </form>
      </div>
  ...
  return (
    <div>
     <div className='text-center mb-3 '>
      <h2>Compra Medicamentos Privado</h2>
      ...
      <table className=" table table-striped table-hover text-sm text-left p-3 col-6">
          ...
          <th scope="col" className="px-6 py-2">Eliminar</th>
          </tr>
        </thead>
        <tbody>
...
export default EntradaPrivadoForm;