...
import { buscarMedicamentosPriv } from '../api/medicamentos';
import { useMovimiento } from '../context/MovimientosContex';


export const SalidaPrivadoForm = () => {
  ...
  const { createSalidaPrivado } = useMovimiento();
  const handleSearch = async (nombre) => {
    console.log('nombre:', nombre)
    try {
      const response = await buscarMedicamentosPriv(nombre); // Pass the nombre directly as a string
  ...
  const handleEntrarSubmit = async (e) => {
    ...
    try {
      const result = await createSalidaPrivado(newMovimiento);
            ...
            <ul>
              {searchResults.map((result) => (
                <li key={result._id} onClick={() => handleSearchResultClick(result.nombre)}>
                  {result.nombre}-stock:{result.stockPriv}
                </li>
              ))}
            </ul>
          )}
  ...
  return (
    <div>
     <div className='text-center mb-3 '>
      <h2>Saída Medicamentos Privado</h2>
