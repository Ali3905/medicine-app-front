...
export const ListaHddForm = () => {
  const { getMedicamentosHDD, editoMedicamento } = useMedicamento();
  ...
  const loadMedicamentos = async () => {
    try {
      const data = await getMedicamentosHDD();
     console.log(data)
 ...
 const sortMedicamentos = () => {
  const sortedMedicamentos = [...medicamentos].sort((a, b) => {
    if (orderField === 'stock') {
      return orderDirection === 'asc' ? a.stockHdd - b.stockHdd : b.stockHdd - a.stockHdd;
...
return (
  <div>
    <div className="text-center text-primary fs-1">Stock Farmacia</div>
